...
class Network(nn.Module):
    def __init__(self, params):
        ...
        if params['type'] == 'lstm':
            ...
            self.w = torch.nn.Parameter((.01 * torch.rand(params['hiddensize'], params['hiddensize'])).cuda(),
                                        requires_grad=True)
            ...
            self.alpha = torch.nn.Parameter((.01 * torch.ones(1)).cuda(),
                                            requires_grad=True)  # Homogenous plasticity: everyone has the same alpha
            self.eta = torch.nn.Parameter((.01 * torch.ones(1)).cuda(), requires_grad=True)  # Everyone has the same eta
        elif params['type'] == 'plastic':
            self.i2h = torch.nn.Linear(TOTALNBINPUTS, params['hiddensize']).cuda()
            self.w = torch.nn.Parameter((.01 * torch.rand(params['hiddensize'], params['hiddensize'])).cuda(),
                                        requires_grad=True)
            self.alpha = torch.nn.Parameter((.01 * torch.rand(params['hiddensize'], params['hiddensize'])).cuda(),
                                            requires_grad=True)
            self.eta = torch.nn.Parameter((.01 * torch.ones(1)).cuda(), requires_grad=True)  # Everyone has the same eta
        ...
        # An LSTM implemented "by hand", to ensure maximum simlarity with the plastic LSTM
            ...
            ##opt = torch.sigmoid(self.x2o(input) + self.h2o(hidden[0]))
    ...
    def forward(self, input, hidden, hebb):
        if self.type == 'lstm':
            # hactiv is just the h. hidden is the h and the cell state, in a tuple
            hactiv, hidden = self.lstm(input.view(1, 1, -1), hidden)
        ...
        # Draft for a "manual" lstm:
            ...
            cell = torch.mul(fgt, hidden[1]) + torch.mul(ipt, torch.tanh(self.x2c(input) + self.h2c(hidden[0])))
            hactiv = torch.mul(opt, torch.tanh(cell))
            # pdb.set_trace()
            hidden = (hactiv, cell)
            ...
            # cell = torch.mul(fgt, hidden[1]) + torch.mul(ipt, torch.tanh(self.x2c(input) + hidden[0].mm(self.w + torch.mul(self.alpha, hebb)))) #  self.h2c(hidden[0])))
            ...
            if self.rule == 'hebb':
                ...
                hebb = hebb + self.eta * torch.mul((hidden[0].unsqueeze(1) - torch.mul(hebb, hactiv[0].unsqueeze(0))),
                                                   hactiv[0].unsqueeze(
                                                       0))  # Oja's rule. Remember that yin, yout are row vectors (dim (1,N)). Also, broadcasting!
            else:
    ...
    def initialZeroState(self):
        if self.params['type'] == 'lstm':
            return (Variable(torch.zeros(1, 1, self.params['hiddensize']), requires_grad=False).cuda(),
                    Variable(torch.zeros(1, 1, self.params['hiddensize']), requires_grad=False).cuda())
            ...
            return (Variable(torch.zeros(1, self.params['hiddensize']), requires_grad=False).cuda(),
                    Variable(torch.zeros(1, self.params['hiddensize']), requires_grad=False).cuda())
...
def train(paramdict):
    ...
    suffix = "maze_" + "".join([str(x) + "_" if pair[0] is not 'nbsteps' and pair[0] is not 'rngseed' and pair[
        0] is not 'save_every' and pair[0] is not 'test_every' else '' for pair in
    ...
    for row in range(1, LABSIZE - 1):
        for col in range(1, LABSIZE - 1):
            if row % 2 == 0 and col % 2 == 0:
                lab[row, col] = 1
    lab[
        CTR, CTR] = 0  # Not really necessary, but nicer to not start on a wall, and perhaps helps localization by introducing a detectable irregularity in the center?
    ...
    for numiter in range(params['nbiter']):
        ...
        if params['rp'] == 0:
            ...
            while lab[rposr, rposc] == 1 or (
                    rposr != 1 and rposr != LABSIZE - 2 and rposc != 1 and rposc != LABSIZE - 2):
                rposr = np.random.randint(1, LABSIZE - 1)
                rposc = np.random.randint(1, LABSIZE - 1)
        ...
        for numstep in range(params['eplen']):
            ...
            inputsN[0, 0:RFSIZE * RFSIZE] = lab[posr - RFSIZE // 2:posr + RFSIZE // 2 + 1,
                                            posc - RFSIZE // 2:posc + RFSIZE // 2 + 1].flatten()
            ...
            # Running the network
            y, v, hidden, hebb = net(Variable(inputs, requires_grad=False), hidden,
                                     hebb)  # y  should output probabilities; v is the value prediction
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bentropy", type=float,
    ...
    parser.add_argument("--randstart", type=int,
                        help="when hitting reward, should we teleport to random location (1) or center (0)?", default=1)
    ...
    parser.add_argument("--print_every", type=int, help="number of cycles between successive printing of information",
                        default=100)
